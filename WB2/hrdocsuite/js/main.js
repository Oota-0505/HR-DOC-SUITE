...
// Problem セクション スタッガードイン（スクロール連動）
...
window.addEventListener('scroll', () => {
  ...
  const heroBgOverlay = document.querySelector('.hero-bg-overlay-gradient');
  ...
  // レイヤー1: 背景グリッド（最も遅い）
  if (heroBgMedia) {
    heroBgMedia.style.transform = `translateY(${scrollPos * 0.08 * parallaxMultiplier}px) scale(${1 + scrollPos * 0.0002 * parallaxMultiplier})`;
  }
  
  // レイヤー2: 背景グラデーション
  if (heroBgOverlay) {
    heroBgOverlay.style.transform = `translateY(${scrollPos * 0.12 * parallaxMultiplier}px)`;
    heroBgOverlay.style.opacity = Math.max(0.3, 1 - scrollPos / 1000);
  }
  
  // レイヤー3: 背景テキスト（中間速度）
  if (heroBgText) {
    heroBgText.style.transform = `translate(-50%, calc(-50% + ${scrollPos * 0.18 * parallaxMultiplier}px)) scale(${1 + scrollPos * 0.0003 * parallaxMultiplier})`;
    heroBgText.style.opacity = Math.max(0, 1 - scrollPos / 600);
  }
  
  // レイヤー4: コンテンツパネル（速め）
  if (glassPanel) {
    glassPanel.style.transform = `translateY(${scrollPos * 0.35 * parallaxMultiplier}px)`;
    glassPanel.style.opacity = Math.max(0, 1 - scrollPos / 700);
  }
  
  // レイヤー5: ビジュアルラップ全体（3D空間）
  if (heroVisualWrap && !isMobile) {
    heroVisualWrap.style.transform = `translateY(${scrollPos * -0.15 * parallaxMultiplier}px) translateZ(${scrollPos * 0.5 * parallaxMultiplier}px)`;
  } else if (heroVisualWrap) {
    heroVisualWrap.style.transform = `translateY(${scrollPos * -0.15 * parallaxMultiplier}px)`;
  }
  
  // レイヤー6: メインビジュアル（最も速い + 3D回転）
  if (heroMainVisual && !isMobile) {
    const rotateAmount = Math.min(scrollPos * 0.02, 15) * parallaxMultiplier;
    heroMainVisual.style.transform = `
      perspective(1200px) 
      rotateY(${-10 + rotateAmount}deg) 
      rotateX(${5 - rotateAmount * 0.3}deg) 
      translateY(${scrollPos * -0.25 * parallaxMultiplier}px)
      translateZ(${scrollPos * 0.3 * parallaxMultiplier}px)
      scale(${1 - scrollPos * 0.0003 * parallaxMultiplier})
    `;
  } else if (heroMainVisual) {
    heroMainVisual.style.transform = `translateY(${scrollPos * -0.25 * parallaxMultiplier}px)`;
  }
  
  // スクロールインジケーター（カーテンロール）のフェードアウト（中央配置を維持）
  if (heroScrollIndicator) {
    const indicatorOpacity = Math.max(0, 1 - scrollPos / 300);
    heroScrollIndicator.style.opacity = indicatorOpacity;
    heroScrollIndicator.style.transform = `translate(-50%, ${scrollPos * 0.5}px)`;
    // 完全に見えなくなったら pointer-events を無効化
    heroScrollIndicator.style.pointerEvents = indicatorOpacity > 0.1 ? 'auto' : 'none';
  }

  // PROBLEM 多層パララックス（奥行き強化）
  ...
  if (problemSection && !isMobile) {
    ...
    // セクションが画面内にあるときのみパララックスを適用
    if (sectionTop < windowHeight && sectionTop + sectionHeight > 0) {
      const sectionScroll = windowHeight - sectionTop;
      const scrollProgress = Math.max(0, Math.min(1, sectionScroll / (windowHeight + sectionHeight)));
      
      // 背景画像のパララックス（強化）
      problemSection.style.backgroundPositionY = `${scrollProgress * -200 * parallaxMultiplier}px`;
      
      // 背景のスケール変化で奥行き感を演出
      const bgScale = 1 + scrollProgress * 0.1 * parallaxMultiplier;
      problemSection.style.backgroundSize = `${bgScale * 100}% auto`;
      
      // Problem ヘッダーとリードの個別パララックス
      const problemHeader = problemSection.querySelector('.problem-header');
      const problemLead = problemSection.querySelector('.problem-lead');
      const empathyGrid = problemSection.querySelector('.empathy-grid');
      
      if (problemHeader) {
        problemHeader.style.transform = `translateY(${sectionScroll * -0.08 * parallaxMultiplier}px)`;
      }
      
      if (problemLead) {
        problemLead.style.transform = `translateY(${sectionScroll * -0.05 * parallaxMultiplier}px)`;
      }
      
      // カードグリッド全体を少し遅めに動かす（奥行き）
      if (empathyGrid) {
        empathyGrid.style.transform = `translateY(${sectionScroll * -0.03 * parallaxMultiplier}px)`;
      }
    }
  }
});
